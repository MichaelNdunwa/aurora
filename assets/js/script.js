...
const $sidebar = $('.main-content .sidebar');

$('#menu-open').on('click', () => $sidebar.css('left', '0'));
$('#menu-close').on('click', () => $sidebar.css('left', '-100%'));
...
async function downloadMusic(trackUrl, trackName) {
    try {
        // Create a temporary anchor element
        const link = document.createElement('a');
        link.href = trackUrl;
        link.download = trackName || 'music-download';
        ...
        alert('Failed to download the music file. Please try again.');
    }
}
